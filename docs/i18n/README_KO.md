...
- launch manifest 에는 `execution_policy` 도 저장됩니다. 이 skill 의 기본값은 `danger_full_access` 이므로, 명시적으로 `workspace_write` 를 요청하지 않는 한 managed cycle 은 보통 `--dangerously-bypass-approvals-and-sandbox` 로 실행됩니다
