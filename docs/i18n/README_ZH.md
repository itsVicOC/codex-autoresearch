...
交互式 skill 依赖两个很小的 Codex 会话 hooks。如果缺失，会在初次扫描仓库后、第一次澄清提问前自动补齐。你也可以手动预装或查看状态：
...
- 当你重新打开或恢复一个 run 时，它会再次给出简短的运行清单
- 当某个 run 其实还应该继续时，它可以避免 Codex 过早停下

需要知道的边界：

- 它们只会影响后续那些明确看起来像 `codex-autoresearch` 工作的 Codex 会话。
- 它们不会改变你眼前这个已经打开的 foreground 会话。
- 它们不会误伤同一仓库里的无关 Codex 对话。
- 如果这是 skill 刚刚在当前会话里自动安装的，`background` 可以立刻受益。
- 如果你希望 foreground 也用上它，需要在**新的 Codex 会话**里重新打开或恢复当前线程。CLI 里这通常是 `codex resume`；桌面端则是在新会话里重新打开同一个线程。
- `background` 模式下，自定义 `--results-path` / `--state-path` 仍然可用。
