...
- For clearly long-running foreground/background runs, check `python3 <skill-root>/scripts/autoresearch_hooks_ctl.py status` before the final mode choice. If hooks are missing, give a short note that helps the user choose intelligently: `background` can benefit immediately if hooks are installed before launch, while the current `foreground` session cannot hot-reload them. After the user chooses a mode, offer installation only when it is useful for that path, and never auto-install without explicit user approval.
